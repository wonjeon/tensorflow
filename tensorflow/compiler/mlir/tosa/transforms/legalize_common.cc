...
static Value buildReshape(ImplicitLocOpBuilder& builder, Value value,
  ...
  auto staticShapeType = RankedTensorType::get(
      {static_cast<int64_t>(staticShape.size())}, builder.getI64Type());
  auto staticShapeAttr = DenseElementsAttr::get(
      staticShapeType,
      llvm::ArrayRef<int64_t>(tensorflow::ConvertMlirShapeToTF(staticShape)));
  auto staticShapeValue = builder.create<tosa::ConstOp>(
...
std::optional<Value> convertSelectOp(PatternRewriter& rewriter, Operation* op,
  ...
  auto reshape_op = CreateOpAndInfer<tosa::ReshapeOp>(
      ...
      condition_value,
      getTosaConstShape(rewriter, op->getLoc(), tensorflow::ConvertMlirShapeToTF(new_cond_dims)));
